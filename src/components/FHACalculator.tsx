...
const getAnnualMIPRate = (loanTerm: number, ltvPercent: number): number => {
  // For loans > 15 years
  if (loanTerm > 15) {
    if (ltvPercent <= 95) return 0.50;
    return 0.55;
  }
  // For loans <= 15 years
  if (ltvPercent <= 90) return 0.15;
  return 0.40;
};

const UPFRONT_MIP_RATE = 1.75; // 1.75% upfront MIP

const FHACalculator = () => {
  const [inputs, setInputs] = useState<FHAInputs>({
    homePrice: 300000,
    downPayment: 10500, // 3.5% minimum for FHA
  ...
  const updateInput = (field: keyof FHAInputs, value: number | string) => {
    setInputs((prev) => {
  ...
  const calculations = useMemo(() => {
    ...
    // FHA requires upfront MIP to be financed into the loan
    const upfrontMIP = baseLoanAmount * (UPFRONT_MIP_RATE / 100);
    const totalLoanAmount = baseLoanAmount + upfrontMIP;
    
    // Annual MIP rate based on loan term and LTV
    const annualMIPRate = getAnnualMIPRate(inputs.loanTerm, ltvPercent);
    const monthlyMIP = (baseLoanAmount * (annualMIPRate / 100)) / 12;

    const monthlyRate = inputs.interestRate / 100 / 12;
    const numPayments = inputs.loanTerm * 12;

    // Monthly principal & interest (on total loan including upfront MIP)
    const monthlyPI =
      monthlyRate > 0
    ...
    const totalMonthly = monthlyPI + monthlyTax + monthlyInsurance + monthlyMIP + monthlyHOA;
    const totalInterest = monthlyPI * numPayments - totalLoanAmount;
    const totalMIP = monthlyMIP * numPayments + upfrontMIP;
    ...
    <div className="fade-in">
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <section className="calculator-card p-3 sm:p-6" aria-labelledby="fha-loan-details-heading">
            <h2 id="fha-loan-details-heading" className="text-base sm:text-xl font-display font-semibold mb-3 sm:mb-6 flex items-center gap-2">
                ...
                <Label htmlFor="down-payment" className="text-xs sm:text-sm text-muted-foreground whitespace-nowrap flex items-center gap-1 min-w-[80px] sm:min-w-[100px]">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>FHA loans require a minimum 3.5% down payment with a credit score of 580+, or 10% with a score of 500-579.</p>
                    </TooltipContent>
                  </Tooltip>
                </Label>
        ...
        <aside className="space-y-6" aria-label="Calculation Results">
          ...
          <div className="calculator-card p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground flex items-center gap-1">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>FHA requires an upfront Mortgage Insurance Premium of 1.75% of the loan amount, which is typically financed into the loan.</p>
                    </TooltipContent>
                  </Tooltip>
                </span>
                <span className="font-medium">{formatCurrency(calculations.upfrontMIP)}</span>
              </div>
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground flex items-center gap-1">
                  Annual MIP ({calculations.annualMIPRate}%)
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Info className="h-3 w-3 text-muted-foreground cursor-help" />
                    </TooltipTrigger>
                    <TooltipContent className="max-w-xs">
                      <p>FHA loans require annual mortgage insurance premiums. For most 30-year loans, this is required for the life of the loan.</p>
                    </TooltipContent>
                  </Tooltip>
                </span>
                <span className="font-medium">{formatCurrency(calculations.monthlyMIP)}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Total MIP (life of loan)</span>
                <span className="font-medium">{formatCurrency(calculations.totalMIP)}</span>
              </div>
            </div>
          </div>
        </aside>
      ...
      <section className="mt-8" aria-labelledby="payment-analysis-heading">
        <Tabs defaultValue="breakdown" className="w-full">
          ...
          <TabsContent value="breakdown">
            <PaymentBreakdown
              principal={calculations.monthlyPI - (calculations.totalLoanAmount * (inputs.interestRate / 100 / 12))}
              interest={calculations.totalLoanAmount * (inputs.interestRate / 100 / 12)}
              tax={calculations.monthlyTax}
              insurance={calculations.monthlyInsurance}
              pmi={calculations.monthlyMIP}
          ...
          <TabsContent value="chart">
            <AmortizationChart 
              ...
              monthlyPMI={calculations.monthlyMIP}
