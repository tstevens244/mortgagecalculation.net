...
// USDA loan fees (as of 2024)
const UPFRONT_GUARANTEE_FEE_RATE = 1.0; // 1% upfront guarantee fee
const ANNUAL_FEE_RATE = 0.35; // 0.35% annual fee

const USDACalculator = () => {
  const [inputs, setInputs] = useState<USDAInputs>({
    homePrice: 250000,
    downPayment: 0, // USDA allows 0% down
  ...
  const updateInput = (field: keyof USDAInputs, value: number | string) => {
    setInputs((prev) => {
  ...
  const calculations = useMemo(() => {
    ...
    // USDA upfront guarantee fee is financed into the loan
    const upfrontGuaranteeFee = baseLoanAmount * (UPFRONT_GUARANTEE_FEE_RATE / 100);
    const totalLoanAmount = baseLoanAmount + upfrontGuaranteeFee;
    
    // Annual fee based on remaining principal balance
    const monthlyAnnualFee = (baseLoanAmount * (ANNUAL_FEE_RATE / 100)) / 12;

    const monthlyRate = inputs.interestRate / 100 / 12;
    const numPayments = inputs.loanTerm * 12;

    // Monthly principal & interest (on total loan including upfront fee)
    const monthlyPI =
      monthlyRate > 0
    ...
    const totalMonthly = monthlyPI + monthlyTax + monthlyInsurance + monthlyAnnualFee + monthlyHOA;
    const totalInterest = monthlyPI * numPayments - totalLoanAmount;
    const totalGuaranteeFees = monthlyAnnualFee * numPayments + upfrontGuaranteeFee;
    ...
    <div className="fade-in">
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <section className="calculator-card p-3 sm:p-6" aria-labelledby="usda-loan-details-heading">
            <h2 id="usda-loan-details-heading" className="text-base sm:text-xl font-display font-semibold mb-3 sm:mb-6 flex items-center gap-2">
                ...
                <Label htmlFor="down-payment" className="text-xs sm:text-sm text-muted-foreground whitespace-nowrap flex items-center gap-1 min-w-[80px] sm:min-w-[100px]">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>USDA loans allow 0% down payment for eligible borrowers in rural areas.</p>
                    </TooltipContent>
                  </Tooltip>
                </Label>
        ...
        <aside className="space-y-6" aria-label="Calculation Results">
          ...
          <div className="calculator-card p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground flex items-center gap-1">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>USDA loans require a 1% upfront guarantee fee, which is typically financed into the loan.</p>
                    </TooltipContent>
                  </Tooltip>
                </span>
                <span className="font-medium">{formatCurrency(calculations.upfrontGuaranteeFee)}</span>
              </div>
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground flex items-center gap-1">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>USDA loans have an annual guarantee fee of 0.35% of the remaining loan balance, paid monthly.</p>
                    </TooltipContent>
                  </Tooltip>
                </span>
                <span className="font-medium">{formatCurrency(calculations.monthlyAnnualFee)}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Total Guarantee Fees</span>
                <span className="font-medium">{formatCurrency(calculations.totalGuaranteeFees)}</span>
              </div>
            </div>
          </div>
        </aside>
        ...
        <Tabs defaultValue="breakdown" className="w-full">
          ...
          <TabsContent value="breakdown">
            <PaymentBreakdown
              ...
              pmi={calculations.monthlyAnnualFee}
          ...
          <TabsContent value="chart">
            <AmortizationChart 
              ...
              monthlyPMI={calculations.monthlyAnnualFee}
